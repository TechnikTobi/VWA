...
%\nocite{ANNADL}																					%Begin Einbinden der Werke
%\nocite{Kriesel2007NeuralNetworks}																	%
%\nocite{3B1B}																						%
\nocite{Practitioner}
\nocite{Nielsen}
\nocite{Gurney}
...
\nocite{LeCunWebseite}
%\nocite{Challenges}
%\nocite{AlphaGo}																					%Ende Einbinden der Werke
...
%\def\oddtest{\ifodd\value{lstnumber}\color{green!5}\else\normalfont\fi}
...
%\newglossaryentry{Machine Learning}{name={Machine Learning},description={Erkennen von Strukturen in Daten durch Algorithmen}}
%\newglossaryentry{Deep Learning}{name=Deep Learning, description={Machine Learning mit KNNs, welche mehr als zwei Hidden Layer haben}, parent={Machine Learning}}
...
\newglossaryentry{Leaky Rectified Linear-Funktion}{name=Leaky Rectified Linear-Funktion, description={Häufige Aktivierungsfunktion, definiert als: $f(x) = \left\{\begin{array}{rcl}
		...
		x&x>0
		\end{array}\right.$}}
%\newglossaryentry{Single-Layer Perceptron}{name=Single-Layer Perceptron, description={KNN mit nur einem Hidden Layer}}
...
\begin{document}
	...
	\begin{titlepage}\label{Titleseite}
		...
		%\vspace{70mm}\Large{\break Abgabedatum: Februar 2019}
...
%		\\ \
%		\\ \
%		Zum Abschluss möchte ich noch ein Zitat von Adam Savage aus der us-amerikanischen TV-Serie „MythBusters“ nennen, an das ich immer wieder bei meinen Experimenten mit KNNs denken musste: „Remember kids, the only difference between science and screwing around is writing it down.“\footnote{text}
		\\ \ 
		\\ \
...
%	\addcontentsline{toc}{chapter}{Einleitung}
	...
	\chapter{Bausteine und Grundlegendes zu Künstlichen Neuronalen Netzwerken}\label{BausteineGrundlegendes}
		...
		\section{Schichten}
		%Wie bereits in \ref{DefKNN} erwähnt werden 
		...
		%\\ \
		%\\ \
		%, wie etwa das Single-Layer Perceptron, 
		...
		\begin{wrapfigure}{r}{0cm}\label{EinBeispielFuerEinKNN}	%Vorlage: Nielsen/Kapitel 1/The architecture of neural networks
...
%			% Kann ich nicht einfach sagen was MNIST ist
		...
		\begin{figure}[h]\label{MNISTbeispiele}
			...
			%\begin{overpic}[height=1cm]{CNN173/3-18.png} \put (85,75) {\footnotesize{\textcolor{white}{3}}} \put (0,75) {\footnotesize{\textcolor{white}{4}}}\end{overpic}
...
%			\end{wrapfigure}
		...
		\enlargethispage{0.2\baselineskip}
			...
			%, wobei $\textrm{len}(L)$ die Anzahl der Neuronen in diesem Layer angibt.
			% da man den Wert $x_{(L,n)}$ als Wahrscheinlichkeit dafür ansehen kann, dass die korrekte Klassifizierung der Fall dieses Neurons ist.
			% Formel \ref{SoftmaxFormel} gibt an, wie die Aktivierung eines Neurons eines Softmax Layers berechnet wird.\footnote{vgl. Nielsen, 2015, Kapitel 3/Softmax}
			\vspace*{-0.3cm}
	...
	\\\
	\begin{wrapfigure}[7]{r}{0cm}	%Vorlage: Olah
		\begin{tikzpicture}[scale=0.45, decoration={markings, mark=at position 1 with {\arrow[scale=2]{>}}}]
	...
	% Gründe für LSTM: Beseitigt Problem des Vanishing Gradients und eine der häufigsten Varianten von Recurrent Neural Networks. (practitioner 149-150)
		...
		% Cell Status
			\begin{wrapfigure}[12]{r}{0cm}	%Vorlage: Olah
				\begin{tikzpicture}[scale=0.5, decoration={markings, mark=at position 1 with {\arrow[scale=2]{>}}}]
				...
				%\draw node[black,midway,yshift=1.5cm,xshift=7cm] at (0,0) {\LARGE $\mathrm{tanh}$};
				...
				%\draw node[black,midway,yshift=5cm,xshift=11.9cm] at (0,0) {\Huge $\textrm{tanh}$};
	...
	%				Backpropagation in Worten (Wartala S. 17)																										
		...
		\section{Gradient Descent}\label{GradientDescent}
			...
			%wie gut das Netzwerk verallgemeinert, d. h.
...
%		% Human Level Performance Deep Learning S. 126
...
%	
		\section{Aufbau und Durchführung}
			...
			%Durchgeführt wurden die Experimente auf einem Heimcomputer mit etwa 48GFLOPs \footnote{BELEG ERFORDERLICH}, einer Leistung welche mit der von Supercomputern der frühen 1990er-Jahren vergleichbar ist.\footnote{BELEG ERFORDERLICH} Dafür wurden insgesamt etwa 15 Stunden Rechenzeit benötigt. %14 stunden und 51 minuten
		...
		\section{Auswertung der Ergebnisse}
			...
			\begin{figure}[h] %Vorlage: Practitioner S. 65-70
				\centering
...
%						\end{tikzpicture}
%					\end{scriptsize}
%					\caption{Mini-Batch Größen bei FFNNs (Quelle: Eigene Darstellung)}
%				\end{minipage}
%				\hfill
%				\begin{minipage}[t]{.32\linewidth}
%					\centering
...
%						\end{tikzpicture}
%					\end{scriptsize}
%					\caption{Mini-Batch Größen bei CNNs (Quelle: Eigene Darstellung)}
%				\end{minipage}
%				\hfill
%				\begin{minipage}[t]{.32\linewidth}
%					\centering
...
%						\end{tikzpicture}
%					\end{scriptsize}
%					\caption{Mini-Batch Größen bei LSTMs (Quelle: Eigene Darstellung)}
%				\end{minipage}
%			\end{figure}
			...
			%\begin{tikzpicture}
			%\begin{axis}[xmin=0,xmax=378]
			%\addplot[mark=none] table [x=achseX, y=quote, col sep=semicolon] {FFNNmodellQuote.csv};
			%\end{axis}
			%\end{tikzpicture}		
			...
			%aufgrund technischer Einschränkungen wurden nur die Sigmoid- und TanH-Funktion verwendet
		...
		\begin{figure}[h]
			...
			\begin{subfigure}[t]{.19\linewidth}
				...
				\caption{6166: 9 (375)}
			\end{subfigure}
			%\begin{overpic}[height=1cm]{CNN173/3-18.png} \put (85,0) {\footnotesize{\textcolor{white}{3}}} \end{overpic}
			\vspace*{-2mm}
		...
		\begin{figure}[H]
			...
			\begin{subfigure}[h]{0.11\linewidth}
				\includegraphics[height=\linewidth]{6zu0/6-21.png}
			\end{subfigure}
			\begin{subfigure}[h]{0.11\linewidth}
				\includegraphics[height=\linewidth]{6zu0/6-439.png}
			\end{subfigure}
			\begin{subfigure}[h]{0.11\linewidth}
				\includegraphics[height=\linewidth]{6zu0/6-3130.png}
			\end{subfigure}
			\begin{subfigure}[h]{0.11\linewidth}
				\includegraphics[height=\linewidth]{6zu0/6-965.png}
			\end{subfigure}
			\unskip\ \vrule\ 
			\begin{subfigure}[h]{0.11\linewidth}
				\includegraphics[height=\linewidth]{6zu0/0-1061.png}
			\end{subfigure}
			\begin{subfigure}[h]{0.11\linewidth}
				\includegraphics[height=\linewidth]{6zu0/0-5415.png}
			\end{subfigure}
			\begin{subfigure}[h]{0.11\linewidth}
				\includegraphics[height=\linewidth]{6zu0/0-1533.png}
			\end{subfigure}
			\begin{subfigure}[h]{0.11\linewidth}
				\includegraphics[height=\linewidth]{6zu0/0-1271.png}
			\end{subfigure}
			%\begin{overpic}[height=1cm]{CNN173/3-18.png} \put (85,75) {\footnotesize{\textcolor{white}{3}}} \put (0,75) {\footnotesize{\textcolor{white}{4}}}\end{overpic}
...
%		\end{acronym}
	...
	\chapter*{Anhang}
		...
		\section*{Anhang A: Notationstabelle}
			...
			\vspace{2mm}
			\newcommand{\newNotationRow}{\\[2.5mm]}
			%\captionof*{table}{Bausteine und Grundlegendes zu Künstlichen Neuronalen Netzwerken}
...
os.chdir("./..")
		...
		\section*{Anhang E: Ergebnisse der Experimente}
			...
			%\csvreader[before reading=\footnotesize, /csv/separator=semicolon,tabular=|r|*{12}{c}|,longtable={|c|c|c|c|c|c|c|c|c|c|c|c|c|},
			%nohead,column count=13,table head=\hline,late after first line=\\\hline,table foot=\hline]{ergebnisse.csv}{}{\csviffirstrow{\myrot{i} & \myrot{ii} & \myrot{iii} & \myrot{iv} & \myrot{v} & \myrot{vi} & \myrot{vii} & \myrot{viii} & \myrot{ix} & \myrot{x} & \myrot{xi} & \myrot{xii} & \myrot{xiii}}{\csvlinetotablerow}}

			
			

			%\csvreader[before reading=\footnotesize, /csv/separator=semicolon,tabular=|r|*{16}{c}|,longtable={|c|c|c|c|c|c|c|c|c|c|c|c|c|c|c|c|c|},
			%nohead,column count=17,table head=\hline,late after first line=\\\hline,table foot=\hline]{ergebnisseCNNfinal.csv}{}{\csviffirstrow{\myrot{i} & \myrot{ii} & \myrot{iii} & \myrot{iv} & \myrot{v} & \myrot{vi} & \myrot{vii} & \myrot{viii} & \myrot{ix} & \myrot{x} & \myrot{xi} & \myrot{xii} & \myrot{xiii} & \myrot{xiv} & \myrot{xv} & \myrot{xvi} & \myrot{xvii}}{\csvlinetotablerow}}
			
			
%			\newpage
%			\normalsize
%			\subsection*{D.3 Ergebnisstabelle für LSTMs}
%			Die Spalte Bias (Wert: True) wurde weggelassen, da ihr Wert für alle Konfigurationen konstant ist.
%			\addcontentsline{toc}{subsection}{D.3 Ergebnisstabelle für LSTMs}
%			\csvreader[before reading=\footnotesize, /csv/separator=semicolon,tabular=|r|*{14}{c}|,longtable={|c|c|c|c|c|c|c|c|c|c|c|c|c|c|c|},
%			nohead,column count=15,table head=\hline,late after first line=\\\hline,table foot=\hline]{ergebnisseLSTMfinal.csv}{}{\csviffirstrow{\myrot{i} & \myrot{ii} & \myrot{iii} & \myrot{iv} & \myrot{v} & \myrot{vi} & \myrot{vii} & \myrot{viii} & \myrot{ix} & \myrot{x} & \myrot{xi} & \myrot{xii} & \myrot{xiii} & \myrot{xiv} & \myrot{xv}}{\csvlinetotablerow}}
...
%			\csvreader[before reading=\footnotesize, /csv/separator=semicolon,tabular=|r|*{12}{c}|,longtable={|c|c|c|c|c|c|c|c|c|c|c|c|c|},
%			nohead,column count=13,table head=\hline,late after first line=\\\hline,table foot=\hline\caption{Tabelle mit den Ergebnissen der Experimente mit FFNNs (Quelle: Eigene Tabelle)}]{ergebnisseFFNNfinal.csv}{}{\csviffirstrow{\myrot{i} & \myrot{ii} & \myrot{iii} & \myrot{iv} & \myrot{v} & \myrot{vi} & \myrot{vii} & \myrot{viii} & \myrot{ix} & \myrot{x} & \myrot{xi} & \myrot{xii} & \myrot{xiii}}{\csvlinetotablerow}}
%			
%			
%			\newpage
%			\normalsize
%			\subsection*{E.2 Tabelle der Ergebnisse der Experimente mit CNNs}
%			Die Spalten Aktivierungsfunktion (Wert: relu), Bias (Wert: True) und Pooling (Wert: Max) wurden weggelassen, da ihr Wert für alle Konfigurationen konstant ist. Diese Werte lieferten bei Vorversuchen jene CNNs mit den höchsten Trefferquoten. 
%			\addcontentsline{toc}{subsection}{E.2 Tabelle der Ergebnisse der Experimente mit CNNs}
%			\csvreader[before reading=\footnotesize, /csv/separator=semicolon,tabular=|r|*{13}{c}|,longtable={|c|c|c|c|c|c|c|c|c|c|c|c|c|c|},
%			nohead,column count=14,table head=\hline,late after first line=\\\hline,table foot=\hline\caption{Tabelle mit den Ergebnissen der Experimente mit CNNs (Quelle: Eigene Tabelle)}]{ergebnisseCNNfinal2.csv}{}{\csviffirstrow{\myrot{i} & \myrot{ii} & \myrot{iii} & \myrot{iv} & \myrot{v} & \myrot{vi} & \myrot{vii} & \myrot{viii} & \myrot{ix} & \myrot{x} & \myrot{xi} & \myrot{xii} & \myrot{xiii} & \myrot{xiv}}{\csvlinetotablerow}}
%			
%			\newpage
%			\normalsize
...
%			
%			\csvreader[before reading=\footnotesize, /csv/separator=semicolon,tabular=|r|*{13}{c}|,longtable={|c|c|c|c|c|c|c|c|c|c|c|c|c|c|}, nohead,
%			column count=14,table head=\hline,late after first line=\\\hline,table foot=\hline\caption{Tabelle mit den Ergebnissen der Experimente mit LSTMs (Quelle: Eigene Tabelle)}]{ergebnisseLSTMfinal2.csv}{}{\csviffirstrow{\myrot{i} & \myrot{ii} & \myrot{iii} & \myrot{iv} & \myrot{v} & \myrot{vi} & \myrot{vii} & \myrot{viii} & \myrot{ix} & \myrot{x} & \myrot{xi} & \myrot{xii} & \myrot{xiii} & \myrot{xiv}}{\csvlinetotablerow}}
%			
	...
	\begin{center}
		...
		\begin{tikzpicture}
			...
			%\draw node[black,midway,yshift=11.5cm,xshift=6.3cm] at (0,0) {\centering Bei Verlust des DVD ist der Inhalt auch verfügbar unter:};
			%\draw node[black,midway,yshift=11.5cm-12pt,xshift=6.3cm] at (0,0) {\centering URL HIER EINFÜGEN};
		\end{tikzpicture}
	\end{center}
