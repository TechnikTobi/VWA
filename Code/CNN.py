    ...
    def __init__(self,
        ...
        for i, hiddenLayer in enumerate(hiddenLayerAufbau):
            if hiddenLayer[0][0] == "Conv":
                if i == 0:
                    self.modell.add(keras.layers.Conv2D(hiddenLayer[1][0],
                                                       (kernelDimensionen[0], kernelDimensionen[1]),
                                                       strides=(strideDimensionen[0], strideDimensionen[1]),
                                                       activation=aktivierungsfunktion,
                                                       padding=hiddenLayer[2][0],
                                                       input_shape=(28,28,1),
                                                       use_bias=self.bias))
                else:
                    self.modell.add(keras.layers.Conv2D(hiddenLayer[1][0],
                ...
                if pooling == "Max":
                    self.modell.add(keras.layers.MaxPooling2D(pool_size=(kernelDimensionen[0], kernelDimensionen[1]),
                                                             strides=(strideDimensionen[0], strideDimensionen[1]),
                                                             padding=hiddenLayer[2][0]))
                elif pooling == "Av.":
                    self.modell.add(keras.layers.AveragePooling2D(pool_size=(kernelDimensionen[0], kernelDimensionen[1]),
                                                                 strides=(strideDimensionen[0], strideDimensionen[1]),
                                                                 padding=hiddenLayer[2][0]))
                ...
                self.modell.add(keras.layers.Dense(hiddenLayer,
                                                  activation=aktivierungsfunktion,
                                                  use_bias=self.bias))
